...
\quad $\bm  f_w(x_i, y_i)\geq \Delta(y_i, y') + \bm f_w(x_i, y') \color{OrangeRed} - \xi_i \color{Green} - \epsilon$\\ % \overbracket[.7pt][.7pt]{\bm w^\top \Psi(x_i, y')}^{\text{mislabelings}} 
...
$\bm f_w(x,y) = w^\top \Psi(x,y)$ : compatibility score btw. $x$ and $y$; \:
$\color{Green} \epsilon$ : tol. / univer. slack variable.

\textbf{Prediction:} \: $c(x) = \argmax_y \bm f_w(x,y)$
\vspace{-3pt}
...
In each iteration, add $\forall(x_i,y_i)$ the constr. with $y' {\neq} y_i$ that is the ``most violated'' and solve again with quadr. opt.
