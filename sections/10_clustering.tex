...
\textbf{Likelihood} $p(x_i | \theta) \to$ \textbf{Conj. prior} $p(\theta | \Theta)$\\
B.lli/(Neg.) Binomial/Geom ${\to}$ Beta\\
Categorical, Multinomial ${\to}$ Dirichlet\\
Normal (known $\sigma^2$) ${\to}$ Normal\\
...
Init. $\Theta^{(0)}$ arbitrarily. For $t=1,2,\ldots$: 1) Sample $\Theta_1^{(t+1)} \sim p(\Theta_1 \mid \Theta_2^{(t)}, \ldots, \Theta_\ell^{(t)}, X)$. 2) Sample $\Theta_2^{(t+1)} \sim p(\Theta_2 \mid \Theta_1^{(t+1)}, \Theta_3^{(t)}, \ldots, \Theta_\ell^{(t)}, X)$. 3) Continue until $\Theta^{(t+1)}_\ell$. \\
